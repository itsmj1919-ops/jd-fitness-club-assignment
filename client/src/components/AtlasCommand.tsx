/**
 * Monumental Athletics: adapted from a 21st.dev command-palette interaction pattern, restyled as architectural wayfinding.
 */
...
const commands = [
  { label: "Enter the Atlas House", hint: "The training world, rooms, and arrival hall", href: "/", keywords: "home world arrival atlas house" },
...
export function AtlasCommand() {
  ...
  return <>
    <button className="atlas-command-trigger" onClick={() => setOpen(true)} aria-label="Open Atlas navigation search"><Search size={15} /><span>Find your practice</span><kbd><Command size={12} />K</kbd></button>
    {open && <div className="atlas-command-backdrop" role="presentation" onMouseDown={() => setOpen(false)}>
      <section className="atlas-command" role="dialog" aria-modal="true" aria-label="Atlas navigation search" onMouseDown={(event) => event.stopPropagation()}>
        <div className="atlas-command-input"><Search size={17} /><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={handleKeys} placeholder="Programs, coaches, membership…" aria-label="Search the Atlas House" aria-activedescendant={matches[selected] ? `atlas-command-${matches[selected].href.replace("/", "root")}` : undefined} /><kbd>ESC</kbd></div>
